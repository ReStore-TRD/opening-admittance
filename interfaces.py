...
def open_create_sheet_dialog_window(spreadsheetId: str) -> Optional[str]:
    dialog_window = tk.Tk()
    dialog_window.tk.call('tk', 'scaling', 8.0)
...
def open_select_sheet_dialog_window(spreadsheetId: str, title: Optional[str] = None) -> Optional[str]:
    ...
    dialog_window = tk.Tk()
    dialog_window.tk.call('tk', 'scaling', 8.0)
    dialog_window.title(f"Select {title or 'sheet'}")
    ...
    tk.Button(dialog_window, text="Accept", command=dialog_window.destroy).grid(row=1, column=0)
...
def open_spreadsheet_dialog_window(title: str, default_value: str) -> Optional[str]:
    dialog_window = tk.Tk()
    dialog_window.tk.call('tk', 'scaling', 2.0)
    ...
    tk.Button(dialog_window, text="Accept", command=dialog_window.destroy).grid(row=1, column=0, columnspan=1)
...
#    create_admittance_window.tk.call('tk', 'scaling', 2.0)
...
def open_column_name_binding_window(target_names: List[str], column_names: List[str]) -> Optional[Dict[str, int]]:
    binding_window = tk.Tk()
    binding_window.tk.call('tk', 'scaling', 2.0)
    binding_window.title("Field binding")

    binding_vars = {arg: tk.StringVar(name=arg, value="Select a column...") for arg in target_names}

    for i, arg in enumerate(target_names):
    ...
    tk.Button(binding_window, text="Accept", command=binding_window.destroy).grid(row=len(target_names), column=1, columnspan=2)
    binding_window.mainloop()

    bindings = {arg: column_names.index(binding_vars[arg].get()) for arg in target_names if
