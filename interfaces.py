...
def open_select_sheet_dialog_window(
    spreadsheetId: str, title: str = "Select Sheet"
) -> Optional[str]:
    try:
        service = build(
            "sheets",
        ...
        sheets = (
            ...
            .get(spreadsheetId=spreadsheetId, fields="sheets.properties")
            .execute()["sheets"]
        )
    except HttpError as e:
        warnings.warn(str(e))
...
def open_spreadsheet_dialog_window(title: str, default_value: str) -> Optional[str]:
    ...
    tk.Label(dialog_window, text="Enter Google Spreadsheet URL:").grid(
...
def open_admittance_creation_window():
    ...
    (listbox := tk.Listbox(listbox_frame, selectmode=tk.SINGLE)).grid(
        row=1, column=0, columnspan=2
    )
    listbox.insert(
        tk.END,
        f"{variable_sets[0]['start_hour'].get()}:{variable_sets[0]['start_minute'].get()}-"
    ...
    def time_changed():
        ...
        time_str = (
            f"{variable_sets[index]['start_hour'].get()}:{variable_sets[index]['start_minute'].get()}-"
    ...
    start_time_hour_spinbox = tk.Spinbox(
        start_time_frame,
        from_=0,
        to=23,
        format="%02.0f",
        textvariable=variable_sets[0]["start_hour"],
        wrap=True,
        width=3,
    ...
    start_time_minute_spinbox = tk.Spinbox(
        start_time_frame,
        from_=0,
        to=59,
        increment=5,
        format="%02.0f",
        textvariable=variable_sets[0]["start_minute"],
        wrap=True,
        width=3,
        command=time_changed,
    )
    ...
    end_time_hour_spinbox = tk.Spinbox(
        end_time_frame,
        from_=0,
        to=23,
        format="%02.0f",
        textvariable=variable_sets[0]["end_hour"],
        wrap=True,
        width=3,
        command=time_changed,
    )
    ...
    end_time_minute_spinbox = tk.Spinbox(
        end_time_frame,
        from_=0,
        to=59,
        increment=5,
        format="%02.0f",
        textvariable=variable_sets[0]["end_minute"],
        wrap=True,
        width=3,
        command=time_changed,
    )
    ...
    num_slots_spinbox = tk.Spinbox(
        timeslot_frame,
        from_=0,
        to=100,
        textvariable=variable_sets[0]["num_slots"],
        width=3,
    )
    ...
    unlimited_checkbox = tk.Checkbutton(
        timeslot_frame, variable=variable_sets[0]["unlimited"], command=change_unlimited
    )
    ...
    def change_selected_timeslot(event_args):
        ...
        start_time_minute_spinbox.config(
            textvariable=variable_sets[index]["start_minute"]
        )
    ...
    def new_timeslot():
        ...
        variable_sets.append(
            {
                ...
                "start_minute": tk.StringVar(
                    name=f"{guid}start_minute", value=end_minute
                ),
                "end_hour": tk.StringVar(
                    name=f"{guid}end_hour", value=str(int(end_hour) + 1)
                ),
        ...
        time_str = "{:02d}:{:02d}-{:02d}:{:02d}".format(
            int(end_hour), int(end_minute), int(end_hour) + 1, int(end_minute)
        )
    ...
    tk.Button(listbox_frame, text="Add", command=new_timeslot, width=7).grid(
        row=2, column=0, sticky=tk.NSEW
    )
    # Create a button to remove the selected item from the list
    tk.Button(listbox_frame, text="Remove", command=remove_timeslot, width=7).grid(
        row=2, column=1, sticky=tk.NSEW
    )

    # spacing
    tk.Frame(create_admittance_window, height=10).grid(
        row=3, column=0, columnspan=4, sticky=tk.NSEW
    )
...
def open_column_name_binding_window(
    datatype: Type, column_names: List[str]
    ...
    binding_vars = {
        arg: tk.StringVar(name=arg, value="Select a column...") for arg in args
    }

    for i, arg in enumerate(args):
        tk.Label(binding_window, text=arg).grid(row=i, column=0)
        tk.OptionMenu(binding_window, binding_vars[arg], *column_names).grid(
            row=i, column=3
        )
    ...
    bindings = {
        arg: column_names.index(binding_vars[arg].get())
        for arg in args
        if binding_vars[arg].get() in column_names
    }
