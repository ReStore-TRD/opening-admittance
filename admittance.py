...
class OpeningAdmittance:
    ...
    def _preprocess_and_mark(self, registrations: Iterable[Registration]):
        ...
        for registration in registrations:
            ...
            if registration.person in self.banned:
                ...
                for banned_person in self.banned:
                    if banned_person.similar(registration.person):
                        if (
                            confirmed_duplicate := registration.person
                            in self.confirmed_duplicates
                        ):
                            self.marked[registration.person].append(
                                f"Confirmed ban, see ban list for {banned_person}!"
                            )
                            self.banned.add(registration.person)
                            break
                        else:
                            self.marked[registration.person].append(
                                f"Suspected ban: {registration.person} might be!, "
            ...
            # earlier opening
            if all(
                registration.person in timeslot.disallowed
            ...
            for timeslot_name, timeslot in self.timeslots.items():
                if (
                    timeslot_name not in registration.timeslots
                ):  # we only care if they signed this timeslot
                    continue
            ...
            if (person := registration.person) in proccessed_for_admission.keys():
                ...
                if set(registration.timeslots) != set(
                    proccessed_for_admission[person].timeslots
                ):
                ...
                for (
                    already_processed_person,
                    already_processed_registration,
                ) in proccessed_for_admission.copy().items():
    ...
    def auto_admit(self, registrations: Iterable[Registration]):
        ...
        for registration in self.processed.values():
            if not any(
                ...
                if wanted_slot in self.timeslots
            ):
        ...
        for name, timeslot in self.timeslots.items():
            with open(
                ...
                for registration in timeslot.spots:
                    writer.writerow(
                        {k: v for k, v in registration.__dict__.items() if k in fields}
                    )

        with open(
            f"./output/waiting_list.csv", "w", newline="", encoding="utf-8"
        ) as file:
    ...
    def write_to_spreadsheet(self, spreadsheetId: str):
        ...
        if self.marked:
            ...
            if "marked" in sheet_names:
                # clear sheet
                sheet.values().clear(
                    spreadsheetId=spreadsheetId, range=f"marked!A1:Z"
                ).execute()
            else:
                # create sheet
                sheet.batchUpdate(
                    spreadsheetId=spreadsheetId,
                    body={
                        "requests": [{"addSheet": {"properties": {"title": "marked"}}}]
                    },
                ).execute()
